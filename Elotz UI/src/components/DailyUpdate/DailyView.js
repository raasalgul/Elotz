...
useEffect(()=>{
  var name=Object.keys(test)[0];
  console.log(test[name]);
  setTopic(name.toUpperCase());
  setTasks(test[name]);
  },[]);
  function handleNext() {
    topicIndex<Object.keys(test).length-1?topicIndex++:topicIndex=0;
    // console.log(`${topicIndex} Length ${Object.keys(test).length}`);
    var name=Object.keys(test)[topicIndex];
    setTopic(name.toUpperCase());
    setTasks(test[name]);
   
  }
  function handlePrevious() {
    topicIndex>0?--topicIndex:topicIndex=Object.keys(test).length-1;
